<template>
    <b-container fluid>
                    ...
                    <tbody>
                      <tr v-for="datos in datosPost" :key="datos.id">
                          ...
                          <td>
                            ...
                            <b-button variant="primary" @click="setData(datos)">EDITAR</b-button></td>
                          <td><b-button variant="primary" @click="modalCompletado(datos)">ACTUALIZAR ESTADO</b-button></td>
                      </tr>
                    </tbody>
                </table>
                </b-tab>
        ...
        <b-modal id="modal-1" ref="modal-1" size="xl" title="Bienvenido al ingreso de tareas">
            <form>
            <div class="row">
        ...
        <b-modal id="modal-2" ref="modal-2" size="xl" title="Bienvenido al modal de edicion de tareas">
            <form>
            <div class="row">
                <div class="col-md-6 mb-3">
                    <label for="nombre">Nombre</label>
        ...
        <b-modal id="modal-4" ref="modal-3" title="Eliminar equipo">
            <h6 class="my-4">
            ¿Desea actualizar a completado el registro?
            </h6>
            <template #modal-footer="{}">
            <b-button
                type="submit"
                variant="primary"
                @click="completar(datos), $bvModal.hide('modal-4')">Completar</b-button>
            <b-button variant="danger" @click="$bvModal.hide('modal-4')"
                >Cancelar</b-button
            >
            </template>
        </b-modal>
        <b-modal id="modal-5" ref="modal-3" title="Eliminar">
            <h6 class="my-4">
            ...
            <b-button
                type="submit"
                variant="primary"
                @click="cancelar(datos), $bvModal.hide('modal-5')">Cancelar</b-button>
...
export default {
  ...
  data () {
    return {
      ...
      apiBase: laraverurl + '/tareas/get'
    }
  },
  mounted () {
    xray.index()
    axios.get(laraverurl + '/tareas/get').then((response) => {
  ...
  methods: {
    modalCompletado (datos) {
      this.form = datos
      this.$bvModal.show('modal-4')
      if (this.form.estado === 'COMPLETADO') {
    ...
    onUpdate () {
      const me = this
      axios.put(laraverurl + '/tareas/' + parseInt(me.form.id), {
    ...
    completado (datos) {
      axios.put(laraverurl + '/tareas/completado/' + parseInt(this.form.id)).then((response) => {
        this.refresh()
        this.getDatos()
      })
    },
    completar () {
      let me = this
      if (this.form.estado === 'PENDIENTE') {
        axios
          .put(laraverurl + '/tareas/completado/' + this.form.id, {
            id: this.form.id
          })
          .then((response) => {
    ...
    cancelar () {
      ...
      if (this.form.estado === 'COMPLETADO') {
        axios
          .put(laraverurl + '/tareas/cancelar/' + this.form.id, {
            id: this.form.id
          })
          .then((response) => {
    ...
    eliminarRegistro (datos) {
      axios.delete(laraverurl + '/tareas/' + datos.id).then((response) => {
